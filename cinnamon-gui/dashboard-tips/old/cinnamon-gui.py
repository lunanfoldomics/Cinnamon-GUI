...
#specimen_name = reactive.Value(None)
specimen_name = None
...
# Function to show notifications and write to the log
...
def extract_cells(annotations_path, image_path, df_classes, output_dir):
    print_log(f"Opening {annotations_path}")
    print_log(f"Opening {image_path}")
    #specimenFileName = (os.path.basename(annotations_path).split('.json')[0])
    ...
    try:
        ...
        for shape in data['shapes']:
            ...
            output_path = os.path.join(output_dir, f'{os.path.basename(image_path)}-{label}-{min_x}-{min_y}-{classified}.png')
...
def create_dataset(image_folder, size=256):
    try:
        ...
        for img_file in png_files:
            ...
            print_log(img_file)
            print(f'Specimen dataset:,{specimen_name}.specimen.{size}X{size}.pickle')

        df = pd.DataFrame(images_array, columns=['X', 'y'])
        df.to_pickle(os.path.join(image_folder, f'{specimen_name}.specimen.{size}X{size}.pickle'))
        print_log("specimen Dataset successfully created.")
...
# Function to get the image encoded in base64
    ...
    ui.navset_tab(
        ui.nav_panel(
            ...
            ui.layout_sidebar(
                ...
                ui.div(
                    ui.output_ui("model_summary_display"),  # Added to display the model summary table
        ...
        ui.nav_panel(
            ...
            ui.layout_sidebar(
                ui.panel_sidebar(
                    ui.row(
                    ui.input_slider("img_index", "Select an image", 0, 1, 0, step=1),  # Set appropriate min and max values
...
def server(input, output, session):
    ...
    # Synchronize the text box value with the seed slider
    @reactive.Effect
    ...
    # Synchronize the seed slider value with the text box
    @reactive.Effect
    ...
    # DATA TAB
    # Reactive effect for dataset loading
    @reactive.Effect
    def load_dataset():
    ...
    def load_model():
        ...
        if model_dir is not None:
            ...
            if model_path.get():
                ...
                # Convert summary text to a DataFrame
                ...
                # Ensure the number of columns matches the structure
    ...
    @reactive.Effect
    def _():
        if input.run_labelme() > 0:  # Monitors if the button is pressed
            subprocess.Popen(["labelme"])  # Opens Labelme
            print("Opening Labelme for defining ROIs and labelling cells of interest.")


    # CEFA: Cell Extration From Annotation
    @reactive.Effect
    ...
    def process_annotation():
        ...
        if annotation_file is not None and specimen_file is not None and df_classes is not None:
            ...
            output_dir = os.path.join(os.getcwd()+'/data/datasets/',f"{specimen_name}.specimen")                        
            ...
            extract_cells(annotations_path, specimen_file_path, df_classes, output_dir)
            create_dataset(output_dir)
            show_notification("Annotation processed.")
            print_log("Annotation processed.")  
            print_log(f"Specimen dataset in: {output_dir}")          
            print("Annotation processed.")  
            print(f"Specimen dataset in: {output_dir}")          
    ...
    # Output function for displaying the saved model plot
    @output
    @render.ui
    ...
    # Force invalidation and recalculation of the output when model or dataset are loaded
    @reactive.Effect
    ...
    # Output function for displaying model info
    @output
    @render.text
    def model_info():
        if model_loaded.get():
    ...
    # Reactive effect for model training
    progress_text_value = reactive.Value("")
    @reactive.Effect
    ...
    async def train_model():
        ...
        try:
            ...
            # We must re-assign these parameters for documenting the log
            optimizer = input.optimizer()
            loss = input.loss()
            learning_rate = input.learning_rate()
            l1 = input.l1()
            ...
            # Define Image Generator for Augumentation
            ...
            # Print used paramteters plus some other info in the log files
            ...
            # Callbacks per early stopping e riduzione del learning rate
            ...
            with ui.Progress(min=0, max=100, session=session) as progress:
                ...
                try:
                    for epoch in range(total_epochs):
                        ...
                        # Check if early stopping stopped training
            ...
            # Automatically save the trained model to disk
            # File names embody date: get date and current hour.
            now = datetime.now()
            
            # Formatting data and hours in a specific format, and
            # generate a name for the model, based on time and seed
            ...
            # Create plot, get base64 string, and save the plot on disk
            ...
            # Copy the log file into the model directory
            ...
            # Save the testing DataFrame in separated pickle file
    ...
    def image_display():
        ...
        if model.get() is not None and X is not None and Y is not None:
            # Pattern regex for checking if the dataset is an external.specimen query
            # If it's a query, only the prediction will be displayed,
    ...
    # REPORT TAB
    # Updated statistics calculation function
    ...
    @reactive.Effect
    def _():
        if input.analyze_btn() > 0:  # Reacts to the button click
            if globals().get('model') is not None and globals().get('X') is not None and globals().get('Y') is not None:
                ...
                if match:
                    # Create a DataFrame for image index and predicted class
    ...
    def stats_plot():
        ...
        if df is not None:
            ...
            if match:
                ...
                # Set X-axis labels with oblique rotation
                plt.xticks(rotation=45)   
                plt.tight_layout()             
                
                # Save the plot as a temporary file in memory
                buffer = BytesIO()
