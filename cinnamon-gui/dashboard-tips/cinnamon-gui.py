...
def testCNN(X, Y, model, classes, img_index, query, feature_mapping):
    ...
    if feature_mapping:
        # Saving feature mapping has been disabled.
        #feature_mapping_Dir = os.path.join('feature_maps')
        #os.makedirs(feature_mapping_Dir, exist_ok=True)
        ...
        # Saving feature mapping has been disabled.
        #with open(feature_mapping_Dir + '/Test-' + str(img_index) + '.png', "wb") as f:
        #    f.write(base64.b64decode(img_str))
...
def extract_model_number(file_path):
    #match = re.search(r'/data/models/(\d{4}\.\d{2}\.\d{2}-\d{2}\.\d{2}\.\d{2}-\d{2})\.model', file_path)
...
def server(input, output, session):
    ...
    async def train_model():
        ...
        try:
            ...
            # Print used parameters plus some other info in the log files
            ...
            # Callbacks per early stopping  and learning rate reduction
            ...
            with ui.Progress(min=0, max=100, session=session) as progress:
                ...
                #show_notification('Training in progress...')
                ...
                try:
                    for epoch in range(total_epochs):
                        ...
                        # Verifica se l'early stopping ha fermato l'addestramento
    ...
    def _():
        if input.analyze_btn() > 0:  # Reacts to the button click
            if globals().get('model') is not None and globals().get('X') is not None and globals().get('Y') is not None:
                ...
                # Convert DataFrame in HTML with styles
